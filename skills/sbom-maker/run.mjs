...
function extractDependencies(parsedLockfile) {
  ...
  // evidence_location must point at where the component actually resolves in the
  // supplied lockfile, per format: the npm v2/v3 `packages` map keys on the
  // install path (e.g. packages["node_modules/express"]), while the classic
  // v1 `dependencies` map keys on the bare package name (dependencies["express"]).
  const processDep = (name, version, pkgLicense, location) => {
  ...
  if (packages && typeof packages === 'object') {
    for (const [path, details] of Object.entries(packages)) {
      ...
      processDep(name, details.version || "unknown", details.license, `packages[${JSON.stringify(path)}]`);
    }
  } else if (dependencies && typeof dependencies === 'object') {
    for (const [name, details] of Object.entries(dependencies)) {
      processDep(name, details.version || "unknown", details.license, `dependencies[${JSON.stringify(name)}]`);
    }
  } else {
