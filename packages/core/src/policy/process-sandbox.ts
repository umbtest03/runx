...
export function prepareLocalProcessSandbox(options: LocalProcessSandboxOptions): LocalProcessSandboxResult {
  ...
  const runtime = resolveSandboxRuntime(declaration.profile, ambientEnv);
...
function buildSandboxMetadata(options: {
  ...
  const networkEnforcement = options.runtime.kind === "bubblewrap"
    ...
    : options.runtime.kind === "direct" ? "host-ambient" : "unsupported";
...
function resolveSandboxRuntime(
  ...
  if (process.platform !== "linux") {
    return {
      kind: "unsupported",
      reason: `local sandbox profile '${profile}' requires Linux bubblewrap support for filesystem and network enforcement`,
    };
  }
  ...
  if (!bwrapPath) {
    return {
      kind: "unsupported",
      reason: `local sandbox profile '${profile}' requires bubblewrap (bwrap) for filesystem and network enforcement`,
    };
  }
